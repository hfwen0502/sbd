...
namespace sbd {
  ...
  /**
     Function to construct an extended half-determinant subspace by generating
     all single-particle AND double-particle excitations from half-determinants
     appearing in dominant determinants (amplitude-filtered).
   */
  template <typename ElemT, typename RealT>
  void SinglesDoublesExtendHalfdets(const std::vector<ElemT> & w,
				     const std::vector<std::vector<size_t>> & adet,
				     const std::vector<std::vector<size_t>> & bdet,
				     size_t bit_length,
				     size_t norb,
    ...
    std::vector<std::vector<size_t>> hdet_singles(max_single_from_a);
    std::vector<std::vector<size_t>> hdet_doubles(max_double_from_a);
    std::vector<int> open_adet(num_vir_a);
    std::vector<int> closed_adet(num_one_a);
    for(size_t ia=0; ia < reduced_adet_size; ia++) {
      int nc = getOpenClosed(new_adet_local[ia],bit_length,norb,open_adet,closed_adet);
      size_t numc = static_cast<size_t>(nc);
      single_from_hdet(new_adet_local[ia],bit_length,norb,numc,open_adet,closed_adet,hdet_singles);
      for(size_t k=0; k < hdet_singles.size(); k++) {
	new_adet_local[ia_count++] = hdet_singles[k];
      }
      double_from_hdet(new_adet_local[ia],bit_length,norb,numc,open_adet,closed_adet,hdet_doubles);
    ...
    hdet_singles.resize(max_single_from_b);
    hdet_doubles.resize(max_double_from_b);
    std::vector<int> open_bdet(num_vir_b);
    std::vector<int> closed_bdet(num_one_b);
    for(size_t ib=0; ib < reduced_bdet_size; ib++) {
      int nc = getOpenClosed(new_bdet_local[ib],bit_length,norb,open_bdet,closed_bdet);
      size_t numc = static_cast<size_t>(nc);
      single_from_hdet(new_bdet_local[ib],bit_length,norb,numc,open_bdet,closed_bdet,hdet_singles);
      for(size_t k=0; k < hdet_singles.size(); k++) {
	new_bdet_local[ib_count++] = hdet_singles[k];
      }
      double_from_hdet(new_bdet_local[ib],bit_length,norb,numc,open_bdet,closed_bdet,hdet_doubles);
      for(size_t k=0; k < hdet_doubles.size(); k++) {
  ...
  /**
     Function to construct the complete half-determinant subspace
     generated by single-particle AND double-particle excitations
     of all half-determinants, without amplitude-based selection.
     This function assumes carryover bitstrings as input.
   */
  void SinglesDoublesExtendHalfdets(const std::vector<std::vector<size_t>> & adet,
				     const std::vector<std::vector<size_t>> & bdet,
				     size_t bit_length,
				     size_t norb,
				     const size_t adet_comm_size,
