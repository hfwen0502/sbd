#mpirun -np 16 -x OMP_NUM_THREADS=1 ./diag --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.txt --method 0 --block 10 --iteration 4 --tolerance 1.0e-4 --adet_comm_size 2 --bdet_comm_size 2 --task_comm_size 2 --init 0 --shuffle 0 --carryover_ratio 0.5 --savename wf --dump_matrix_form_wf matrixformwf.txt --carryoverfile carryover.txt --rdm 0
#mpirun -np 16 -x OMP_NUM_THREADS=1 ./diag --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.txt --method 0 --block 10 --iteration 4 --tolerance 1.0e-4 --adet_comm_size 2 --bdet_comm_size 2 --task_comm_size 2 --init 0 --shuffle 0 --carryover_ratio 0.5 --loadname wf --rdm 1

#mpirun -np 1 -x OMP_NUM_THREADS=4 ./build/sbd --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.txt --method 0 --block 10 --iteration 1 --tolerance 1.0e-4 --adet_comm_size 1 --bdet_comm_size 1 --task_comm_size 1 --init 0 --shuffle 0 --carryover_ratio 0.5 --savename wf --dump_matrix_form_wf matrixformwf.txt --carryoverfile carryover.txt --rdm 0

# run small size
mpirun --hostfile hostfile -np 1 ./diag --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.txt --method 2 --block 10 --iteration 1 --tolerance 1.0e-4 --adet_comm_size 1 --bdet_comm_size 1 --task_comm_size 1 --init 0 --shuffle 0 --carryover_ratio 0.5 --savename wf --dump_matrix_form_wf matrixformwf.txt --carryoverfile carryover.txt --rdm 0

# run 6k x 6k
# mpirun --hostfile hostfile -np 1 ./diag --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.bin --method 0 --block 10 --iteration 2 --tolerance 1.0e-4 --eps 1.0e-4 --adet_comm_size 1 --bdet_comm_size 1 --task_comm_size 1 --init 0 --shuffle 0 --carryover_ratio 0.5 --savename wf --carryoverfile carryover.txt --rdm 0 --max_time 24000


# run small without pre-calculation
#mpirun --hostfile hostfile -np 1 ./diag --fcidump fcidump_Fe4S4.txt --adetfile AlphaDets.txt --method 0 --block 10 --iteration 1 --tolerance 1.0e-4 --adet_comm_size 1 --bdet_comm_size 1 --task_comm_size 1 --init 0 --shuffle 0 --carryover_ratio 0.5 --savename wf --dump_matrix_form_wf matrixformwf.txt --carryoverfile carryover.txt --rdm 0 --use_precalculated_dets 0

